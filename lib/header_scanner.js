...
function mapNetworkError(err) {
  ...
  const tlsCodes = [
    ...
    'ERR_SSL_SSLV3_ALERT_HANDSHAKE_FAILURE', 'ERR_SSL_TLSV1_UNRECOGNIZED_NAME'
  ];
  if (tlsCodes.includes(code) || code.startsWith('ERR_SSL') || code.startsWith('ERR_TLS')) {
    return new ScanError('tls_error', 'El certificado o la conexión TLS de ese dominio falló la verificación.', code);
  }
  if (code === 'ECONNREFUSED') return new ScanError('unreachable', 'El servidor rechazó la conexión HTTPS (puerto 443 cerrado).', code);
  if (code === 'ETIMEDOUT' || code === 'ESOCKETTIMEDOUT') return new ScanError('timeout', 'El servidor tardó demasiado en responder.', code);
  return new ScanError('unreachable', 'No pudimos conectar con ese dominio por HTTPS.', code);
}
...
function requestOnce(hostname, pinned, deps, timeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({
      ...
      timeout: timeoutMs,
      headers: {
        'User-Agent': 'UnblockAI-Shield-HeaderCheck/1.0 (+https://unblock-shield.vercel.app)',
...
async function fetchHeaders(startHost, deps) {
  ...
  for (;;) {
    ...
    const res = await requestOnce(host, pinned, deps, Math.min(REQUEST_TIMEOUT_MS, deadline - Date.now()));
